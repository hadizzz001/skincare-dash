...
export async function GET(req) {
  try {
    const categories = await prisma.banner2.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.banner2.create({ data: {  img } });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.banner2.update({
      where: { id },
      data: {  img  },
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.banner2.delete({ where: { id } });
