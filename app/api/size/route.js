...
export async function POST(req) {
  try {
    ...
    const category = await prisma.size.create({ data: { title} });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.size.update({
      where: { id },
      data: { title},
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.size.delete({ where: { id } });
