...
export async function GET(req) {
  try {
    const categories = await prisma.factory.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.factory.create({ data: { name, img } });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.factory.update({
      where: { id },
      data: { name, img },
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.factory.delete({ where: { id } });
