...
export async function PATCH(request, { params }) {
  ...
  try {
    ...
    const qtyToAdd = parseInt(qty, 10);

    if (isNaN(currentStock) || isNaN(qtyToAdd)) {
      return new Response(JSON.stringify({ error: 'Invalid stock or quantity' }), { status: 400 });
    }

    const updatedStock = String(currentStock + qtyToAdd);
