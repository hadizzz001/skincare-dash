...
export async function GET(req) {
  try {
    const categories = await prisma.txt.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.txt.create({ data: { title} });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.txt.update({
      where: { id },
      data: { title},
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.txt.delete({ where: { id } });
