...
export async function GET(req) {
  try {
    const categories = await prisma.hotoffer.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.hotoffer.create({ data: { name, img } });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.hotoffer.update({
      where: { id },
      data: { name, img },
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.hotoffer.delete({ where: { id } });
