...
export async function GET(req) {
  try {
    const categories = await prisma.banner1.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.banner1.create({ data: {  img } });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.banner1.delete({ where: { id } });
